...
# Modify 1:import time calculation
import time
import collections
import csv
import os
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    elapsed = time.time() - start
    print("evaluation finished, time used:{},average {} per sample".format(elapsed, elapsed/len(eval_examples)))
  ...
  if FLAGS.do_predict:
    ...
    # Modify 2:start predict time calculation
    t = time.time()
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        num_written_lines += 1
    elapsed = time.time() - t
    print("predict finished, time used:{},average {} per sample".format(elapsed, elapsed/len(predict_examples)))
